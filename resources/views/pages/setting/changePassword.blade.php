...
@php
   if ($profile->role->role === "admin")
   {
     ...
     $data=['Dashboard'=>route('dashboard'),'Profile settings'=>route('admin.profile'),'Change Password'=>'#'];
   }
   elseif ($profile->role->role === "vendor")
   {

     $data=['Dashboard'=>route('vendor.dashboard'),'Profile settings'=>route('vendor.profile'),'Change Password'=>'#'];
   }
            ...
            <form action="{{route('password.change',$profile->id)}}" method="POST">
                ...
                <div class="d-flex mt-4">
                    <div class="mx-auto">
                        ...
                        <a href="{{$profile->role->role==="admin"?route('admin.profile'):route('vendor.profile')}}" class="btn btn-danger mr-4">Exit</a>
                    </div>
                </div>
            </form>
        </div>
